...
Tinycon.setBubble = function(n) {
  ...
  if (!faviconUpdateTimer) {
    faviconUpdateTimer = setTimeout(function () {
      clearTimeout(faviconUpdateTimer);
...
var isNewLine = new RegExp('[0-9]{4}-[0-9]{2}-[0-9]{2}.*');
...
window.App = (function app(window, document) {
  ...
  return {
    ...
    log: function log(data) {
      ...
      if (!isNewLine.test(data) && _logContainer.lastChild) {
          const div = _logContainer.lastChild;
          const tail = this._createSpan('tail', data);
          p.appendChild(tail);
          div.appendChild(p);
          ...
          const dataTail = data.substr(idx);
          const head = this._createSpan('head', dataHead);
          const tail = this._createSpan('tail', dataTail);
    ...
    _createSpan: function(className, content) {
      const span = document.createElement('span');
      span.className = className;
      span.innerHTML = _highlightWord(content);
      return span;
    },
  };
}(window, document));
